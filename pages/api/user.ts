...
import { searchUser, updateUser, addUser } from 'lib/api/user';
...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    const { userName, secUserName, email } = req.body;
    // const session = await getSession({ req });
    // if (!session || session.username !== username) {
    //   return res.status(401).json({
    //     error: 'Unauthorized'
    ...
    // }
    try {
      const result = await addUser(userName, secUserName, email);
      // if (result) {
      //   await res.revalidate(`/${username}`);
      // }
      // const bioMdx = await getMdxSource(bio); // return bioMdx to optimistically show updated state
      return res.status(200).json({ttext:'ok'});
    } catch (e: any) {
      console.log(e);
      return res.status(500).json({
        error: e.toString()
